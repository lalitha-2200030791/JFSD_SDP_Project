...
<%
    // Get the session
    HttpSession userSession = request.getSession(); // Renamed variable to avoid duplication
    String username = (String) userSession.getAttribute("username"); // Retrieve the username from the session
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
<%
        // Include user header based on session
        if (username != null) {
    %>
            <jsp:include page="userheader.jsp" /> <!-- Correctly include userheader.jsp -->
    <%
        } else {
    %>
            <jsp:include page="header.jsp" /> <!-- Correctly include header.jsp -->
    <%
        }
    %>
